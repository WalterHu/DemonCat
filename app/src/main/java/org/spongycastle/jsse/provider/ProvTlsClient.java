...
import org.spongycastle.tls.CertificateStatus;
    ...
    implements ProvTlsPeer
{
    ...
    // Removed by hubohua for OCSP Stapling Response begin
//    @Override
//    protected CertificateStatusRequest getCertificateStatusRequest()
//    {
//        return null;
//    }
    // Removed by hubohua for OCSP Stapling Response end
        ...
        return new TlsAuthentication()
        {
            ...
            public void notifyServerCertificate(TlsServerCertificate serverCertificate) throws IOException
            {
                ...
                else
                {
                    ...
                    // Added by hubohua for OCSP Stapling feature begin
                    // 1. Get status response and analysis
                    // 2. Throw TlsFatalAlert if status is not Good
                    CertificateStatus status = serverCertificate.getCertificateStatus();
                    Log.d(ProvTlsClient.class.getSimpleName(), "notifyServerCertificate -> status: " +
                            status);
                    if (status != null) {
                        // status type 1 means ocsp
                        short statusType = status.getStatusType();
                        OCSPResponse response = status.getOCSPResponse();
                        Log.d(ProvTlsClient.class.getSimpleName(), "notifyServerCertificate -> statusType: " +
                                statusType + ", response: " + response);
                        if (response != null) {
                            OCSPResponseStatus ocspResponseStatus = response.getResponseStatus();
                            ResponseBytes responseBytes = response.getResponseBytes();
                            if (ocspResponseStatus != null) {
                                BigInteger value = ocspResponseStatus.getValue();
                                // value 1 means response successfully
                                Log.d(ProvTlsClient.class.getSimpleName(), "notifyServerCertificate -> value: " +
                                        value);
                            }
                            if (responseBytes != null) {
                                ASN1ObjectIdentifier asn1ObjectIdentifier = responseBytes.getResponseType();
                                ASN1OctetString asn1OctetString = responseBytes.getResponse();
                                Log.d(ProvTlsClient.class.getSimpleName(), "notifyServerCertificate -> asn1ObjectIdentifier: " + asn1ObjectIdentifier.getId());
                                // parse the ocsp response by BasicOCSPResponse
                                BasicOCSPResponse basicOCSPResponse = BasicOCSPResponse.getInstance(asn1OctetString.getOctets());
                                Log.d(ProvTlsClient.class.getSimpleName(), "notifyServerCertificate -> basicOCSPResponse: " +
                                        basicOCSPResponse);
                                BasicOCSPResp basicOCSPResp = new BasicOCSPResp(basicOCSPResponse);
                                SingleResp[] singleResps = basicOCSPResp.getResponses();
                                Log.d(ProvTlsClient.class.getSimpleName(), "notifyServerCertificate -> singleResps: " +
                                        singleResps);
                                if (singleResps != null && singleResps.length > 0) {
                                    SingleResp singleResp = singleResps[0];
                                    if (singleResp != null) {
                                        org.spongycastle.cert.ocsp.CertificateStatus certificateStatus =
                                                singleResp.getCertStatus();
                                        Log.d(ProvTlsClient.class.getSimpleName(), "notifyServerCertificate -> certificateStatus: " +
                                                certificateStatus);
                                        if (certificateStatus != null && certificateStatus instanceof RevokedStatus) {
                                            Date date =
                                                    ((RevokedStatus) certificateStatus).getRevocationTime();
                                            Log.d(ProvTlsClient.class.getSimpleName(), "notifyServerCertificate -> revoke date: " +
                                                    date);
                    ...
                    // Added by hubohua for OCSP Stapling feature end
                }
            }
        };
    }
